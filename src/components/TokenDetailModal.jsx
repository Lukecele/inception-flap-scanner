...
// BSC Swap event topic (PancakeSwap V2)
...
const TokenDetailModal = ({ token, onClose }) => {
  ...
  // ── Pool info ─────────────────────────────────────────────────────────────
  // Fetch pool address from token info if not available
  useEffect(() => {
      ...
      try {
        ...
        if (tradeType === 'buy') {
          ...
          if (!raw || raw <= 0 || isNaN(raw)) {
            ...
            setQuoteError('Inserisci un importo BNB valido (es: 0.1)');
            return;
          }
        ...
        if (d?.code === 'CF_BLOCKED') {
          setQuoteError('GMGN bloccato (Cloudflare) — swap disponibile, preventivo non calcolabile');
          setLoadingQuote(false);
          return;
        }
        if (d?.data?.quote) {
          ...
          setQuoteError(d?.msg || d?.error || 'Route GMGN non disponibile per questo token');
        }
      } catch (e) {
  ...
  const fetchTrades = useCallback(async (pool, sinceBlock) => {
    try {
      ...
      const tokenBlock = token.block ? Number(token.block) : null;
      const hasBlock = tokenBlock && !isNaN(tokenBlock);

      // If pool is migrated, try fetching from GeckoTerminal API to avoid RPC limits
        ...
        try {
          ...
          if (json.data && Array.isArray(json.data)) {
            const parsedTrades = json.data.map(item => {
              ...
              const fromIsWbnb = attr.from_token_address.toLowerCase() === '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c';
          ...
          console.warn("GeckoTerminal fetch failed, falling back to RPC:", e.message);
        }
      }

      // Fallback/Bonding curve token: query RPC eth_getLogs with a block range suited to whether wallet is connected
      let fromHex, toHex;
      if (sinceBlock) {
        fromHex = '0x' + sinceBlock.toString(16);
        toHex = '0x' + latest.toString(16);
      } else {
        const isWalletActive = typeof window !== 'undefined' && window.ethereum && walletAddress;
        if (isWalletActive) {
          // Query last 299 blocks (~15 minutes lookback) to show rich recent trade history
          const fromBlock = Math.max(0, latest - 299);
          fromHex = '0x' + fromBlock.toString(16);
          toHex = '0x' + latest.toString(16);
        } else {
          // Query last 299 blocks, will fall back to 9 blocks if public RPC throws limits error
          const fromBlock = Math.max(0, latest - 299);
          fromHex = '0x' + fromBlock.toString(16);
          toHex = '0x' + latest.toString(16);
        }
      ...
      let rawLogs = [];
      const numBatches = 15; // 150 blocks total (~7.5 minutes of trades)
      const batchPromises = [];
      const targetAddress = (pool && pool !== '0x0000000000000000000000000000000000000000') 
        ? pool.toLowerCase() 
        : token.tokenAddress.toLowerCase();
      const targetTopic = (pool && pool !== '0x0000000000000000000000000000000000000000')
        ? '0xd78ad95fa46c994b6551d0da85fc275fe613ce37657fb8d5e3d130840159d822'
        : '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef';

      for (let i = 0; i < numBatches; i++) {
        const toBlock = latest - (i * 10);
      ...
      try {
        ...
        console.error("Failed to fetch batched logs:", err.message);
      }

      if (!rawLogs || !Array.isArray(rawLogs)) return { trades: [], latestBlock: latest };

      let parsedTrades = [];
      if (pool && pool !== '0x0000000000000000000000000000000000000000') {
        const t0isToken = token.tokenAddress.toLowerCase() < '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c';
        ...
        parsedTrades = rawLogs.map(l => {
          ...
          const isBuy = fromLower === '0x0000000000000000000000000000000000000000' || 
                        fromLower === factoryLower || 
                        fromLower === tokenLower;
          
          const type = isBuy ? 'BUY' : 'SELL';
          const wallet = isBuy ? toAddr : fromAddr;
  ...
  useEffect(() => {
    ...
    const poll = async (since) => {
      ...
      if (result) {
        ...
        setLiveTrades(prev => {
          ...
          return [...newTrades, ...prev].slice(0, 200); // keep max 200
        });
        setLatestBlock(lb);
    ...
    tradesIntervalRef.current = setInterval(async () => {
      currentBlock = await poll(currentBlock ? currentBlock - 5 : null);
    }, 8000);
  ...
  // ── Execute Trade ─────────────────────────────────────────────────────────
  // Single MetaMask transaction — no separate dev fee tx to avoid "cancel pending"
  const executeMetaMaskTrade = async () => {
    ...
    try {
      // 1. Chain check
      const chainId = await window.ethereum.request({ method: 'eth_chainId' });
      if (chainId !== '0x38') throw new Error(`Sei su chain ${chainId}. Passa a BSC (BNB Chain) in MetaMask!`);

      if (tradeType === 'buy') {
        const raw = parseFloat(String(buyAmount).replace(',', '.').trim());
        if (!raw || isNaN(raw) || raw <= 0) throw new Error('Importo BNB non valido — inserisci un numero (es: 0.1)');
        const bnb = parseFloat(bnbBalance);
        if (raw > bnb - 0.002) throw new Error(`Saldo insufficiente: hai ${bnb} BNB, tieni almeno 0.002 per il gas`);

        const DEV_FEE_ADDRESS = '0xafF5340ECFaf7ce049261cff193f5FED6BDF04E7';
        const DEV_FEE_PERCENT = 0.01; // 1% dev fee
        ...
        addLog(`1. Recupero route per ${swapAmount.toFixed(5)} BNB (dedotta dev fee 1%) → ${token.tokenSymbol}…`);
        ...
        if (routeData?.code === 'CF_BLOCKED' || !routeData?.data?.raw_tx) {
          const errMsg = routeData?.error || routeData?.msg || 'Nessuna route disponibile';
          // If token has no DEX pool yet (still on bonding curve), direct to flap
          if (token.launchpadProgress < 1) {
            addLog(`⚠️  Token ancora su Flap Bonding Curve. Apri Flap.sh per acquistare.`);
        ...
        const outTokens = (Number(BigInt(outRaw)) / 1e18).toLocaleString('en-US', { maximumFractionDigits: 0 });
        addLog(`2. Stima: ricevi ~${outTokens} ${token.tokenSymbol}. Firma acquisto in MetaMask…`);

        const rt = routeData.data.raw_tx;
        // Convert value to hex (GMGN may return decimal string)
        const txValue = rt.value
          ? (rt.value.startsWith('0x') ? rt.value : '0x' + BigInt(rt.value).toString(16))
          : '0x0';
        ...
        addLog(`   Attesa conferma transazione su blockchain…`);
        ...
        if (!receipt || receipt.status === '0x0') {
          throw new Error('L\'acquisto del token è fallito o è stato annullato su blockchain.');
        }

        addLog(`   🎉 Acquisto confermato con successo!`);
        await updateBalances(walletAddress);

        // Send dev fee transaction sequentially
        if (feeWei > 0n) {
          try {
            addLog(`3. Invio dev fee (1%): ${feeAmount.toFixed(6)} BNB…`);
            ...
            addLog(`   Attesa conferma dev fee…`);
            
            // Wait for fee receipt
            for (let i = 0; i < 30; i++) {
              const r = await window.ethereum.request({ method: 'eth_getTransactionReceipt', params: [feeTxHash] });
              if (r) break;
              await new Promise(ok => setTimeout(ok, 2000));
            }
            addLog(`   ✅ Dev fee confermata! Grazie per il supporto.`);
          } catch (feeErr) {
            addLog(`⚠️  Dev fee non inviata: ${feeErr.message}`);
          }
          await updateBalances(walletAddress);
        }
        ...
        if (routeData?.code === 'CF_BLOCKED' || !routeData?.data?.raw_tx) {
          ...
          throw new Error(routeData?.error || routeData?.msg || 'Nessuna sell route');
        }

        const rt = routeData.data.raw_tx;

        // Approve router to spend tokens (required for ERC20 sell)
        addLog(`2. Approvazione token per router ${rt.to.slice(0,10)}…`);
        const approveData = '0x095ea7b3'
          + rt.to.toLowerCase().replace('0x', '').padStart(64, '0')
          + sellAmt.toString(16).padStart(64, '0');

        const approveTxHash = await window.ethereum.request({
          method: 'eth_sendTransaction',
          params: [{ from: walletAddress, to: token.tokenAddress, data: approveData }]
        });
        addLog(`   Approvazione inviata: ${approveTxHash.slice(0,14)}… attesa on-chain…`);

        // Wait for approval
        for (let i = 0; i < 40; i++) {
          const r = await window.ethereum.request({ method: 'eth_getTransactionReceipt', params: [approveTxHash] });
          if (r) {
            if (r.status === '0x0') throw new Error('Approvazione fallita. Riprova.');
            break;
          }
          await new Promise(ok => setTimeout(ok, 2000));
        }
        ...
        addLog(`✅ Vendita inviata! TX: ${swapTxHash}`);
        addLog(`   Attesa conferma vendita su blockchain…`);

        // Wait for sell swap tx confirmation
        ...
        if (!swapReceipt || swapReceipt.status === '0x0') {
          throw new Error('La vendita del token è fallita o è stata annullata su blockchain.');
        }

        addLog(`   🎉 Vendita confermata con successo!`);
        await updateBalances(walletAddress);

        // Send dev fee transaction based on estimated proceeds (sells receive BNB)
        const quote = routeData.data.quote;
        const outRaw = quote?.out_amount ?? quote?.output_amount ?? quote?.buy_amount ?? '0';
        const estimatedBnb = Number(BigInt(outRaw)) / 1e18;
        const feeAmount = estimatedBnb * DEV_FEE_PERCENT;
        const feeWei = BigInt(Math.round(feeAmount * 1e18));
    ...
    return (
      <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', color: 'var(--text-bright)' }}>
        <td style={{ padding: '6px 8px', fontFamily: 'monospace', fontSize: '0.68rem' }}>
          <a href={`https://bscscan.com/address/${item.address}`} target="_blank" rel="noreferrer"
            style={{ color: 'var(--accent-cyan)', textDecoration: 'none' }} onClick={e => e.stopPropagation()}>
        ...
        <td style={{ padding: '6px 8px', textAlign: 'right', fontFamily: 'monospace', fontSize: '0.68rem', color: 'var(--accent-cyan)' }}>
          {(item.amount_percentage * 100).toFixed(2)}%
        </td>
    ...
    <div className="detail-modal-overlay animate-fade-in" onClick={onClose}>
      <div className="detail-modal-card animate-scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-header">
          ...
          {/* CLOSE BUTTON — always visible, fixed layout */}
          <button
            className="close-btn"
            onClick={onClose}
          ...
          <div className="modal-chart-column">
            ...
            {activeTab === 'chart' && (
              <div className="chart-area">
                <iframe
                  src={`https://www.gmgn.cc/kline/bsc/${token.tokenAddress}?theme=dark`}
                  title="GMGN Kline Chart"
                ...
                <div className="links-row">
                  {[
                    ...
                    <a key={label} href={href} target="_blank" rel="noreferrer"
                      className="modal-chip" onClick={e => e.stopPropagation()}>{label}</a>
                  ))}
                </div>
              </div>
            )}
            ...
            {/* LIVE TRADES */}
            {activeTab === 'trades' && (
              <div className="chart-area" style={{ gap: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <span style={{ fontSize: '0.62rem', color: 'var(--text-muted)' }}>Aggiornamento ogni 8s</span>
                  </div>
                </div>
                ...
                {loadingTrades && liveTrades.length === 0 ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '2.5rem 1rem', color: 'var(--text-muted)', fontSize: '0.75rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                    <span>Nessuna transazione recente trovata su questa bonding curve.</span>
                    {!walletAddress && (
                      <span style={{ color: 'var(--accent-yellow)', fontSize: '0.7rem' }}>
                        🔌 Connetti MetaMask in alto a destra per sincronizzare lo storico completo on-chain!
                      </span>
                    )}
                  </div>
                ) : (
                  <div style={{ overflowY: 'auto', flex: 1 }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', fontSize: '0.64rem', color: 'var(--text-muted)', position: 'sticky', top: 0, background: 'rgba(11,12,16,0.97)' }}>
                      ...
                      <tbody>
                        {liveTrades.map((t, i) => {
                          ...
                          const bnbAmt = t.bnbAmount && t.bnbAmount !== '0'
                            ? (Number(BigInt(t.bnbAmount)) / 1e18).toFixed(4)
                            : '—';
                          ...
                          return (
                            <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', fontSize: '0.68rem' }}>
                              <td style={{ padding: '5px 6px' }}>
                                <span style={{
                                  background: isBuy ? 'rgba(0,255,135,0.12)' : 'rgba(255,78,100,0.12)',
                                  color: isBuy ? 'var(--accent-green)' : 'var(--accent-red)',
                                  fontWeight: 800, fontSize: '0.62rem', padding: '2px 6px', borderRadius: 3,
                              ...
                              <td style={{ padding: '5px 6px', fontFamily: 'monospace', color: 'var(--text-muted)' }}>
                                <a href={`https://bscscan.com/tx/${t.tx}`} target="_blank" rel="noreferrer"
                                  style={{ color: 'var(--accent-cyan)', textDecoration: 'none' }} onClick={e => e.stopPropagation()}>
            ...
            {/* HOLDERS */}
            {activeTab === 'holders' && (
              <div className="holders-tab-content">
                {loadingHolders ? <Spinner msg="Caricamento Top Holders da GMGN…" /> : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <TableHead5 cols={['Wallet', 'Tag', 'Bilancio', '%', 'P/L BNB']} />
                    <tbody>
                      {holders.length === 0
                        ? <tr><td colSpan={5} style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.72rem' }}>Nessun dato</td></tr>
            ...
            {/* TRADERS */}
            {activeTab === 'traders' && (
              <div className="traders-tab-content">
          ...
          <div className="modal-trading-column">
            ...
            <div className="modal-box">
              ...
              <div className="input-panel" style={{ marginTop: 6 }}>
                ...
                <input type="number" inputMode="decimal" min="1" max="100"
                  value={slippage} onChange={e => setSlippage(e.target.value)}
                  placeholder="15" className="slippage-input" />
              </div>
